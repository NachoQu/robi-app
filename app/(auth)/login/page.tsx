...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // If signIn redirects, we never get here; only set error if returned
    if (result?.error) {
      ...
      >
        <div className="flex flex-col items-center gap-3 mb-6">
          ...
          <p className="text-base text-muted-foreground text-center font-medium">
            Iniciá sesión para ver tus puntos y premios
          </p>
        </div>
          ...
          <CardContent className="px-8 pb-8">
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <motion.div whileTap={{ scale: 0.97 }} className="mt-2">
                ...
                >
                  {loading ? '⏳ Ingresando…' : '✨ ¡Entrar a Robi!'}
                </Button>
              </motion.div>
            </form>
        ...
        <p className="text-center text-xs text-muted-foreground mt-4 font-medium">
          🌟 Aprendé, jugá y ganate premios con Robi
        </p>
      </motion.div>
    </div>
  )
